...
$("html").attr("lang", localeFile.cCode);
    ...
    // Headings
    guildName.html(`<img class="avatarIMG" src="" alt=""> ${localeFile.headings.guildName}`);
    ...
    // Buttons
    refreshToken.html(`${localeFile.buttons.editToken}<i class="ri-logout-circle-r-line float-right text-muted"></i>`);
    ...
    function updateGuild() {
        ...
        if (guilds.val() === "DM") {
            ...
            // Roles button
            html += `<button onclick='$("#guildRoles").toggle("fast")'>${localeFile.infos.roles}</button><div id="guildRoles" style="display:none;">${guild.roles.cache.map((role) => `${escapeHtml(role.name)} (${role.id})`).join("<div style='margin: 8px 0 8px 0'></div>")}</div>`;

            // Channels button
            if (guild.channels.cache.size > 0) {
                html += `<button onclick='$("#guildChannels").toggle("fast")'>${localeFile.infos.channels}</button><div id="guildChannels" style="display:none;">${guild.channels.cache.map((channels) => `${escapeHtml(channels.name)} (${channels.id})`).join("<div style='margin: 8px 0 8px 0'></div>")}</div>`;
            }
    ...
    client.on("ready", () => {
        ...
        botavatar.html(`<a href="${client.user.avatarUrl}" target="_blank"><img alt="" src="${client.user.avatarUrl}" class="avatarIMG"/></a> ${escapeHtml(client.user.username)}`);
